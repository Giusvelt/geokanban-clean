                            ...
                            {activeTab === 'geofences' && (
                                <>
                                        ...
                                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-3">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div className="flex bg-surface-low/20 p-1 rounded-full border border-surface-low/30 shadow-inner">
                                                    <button
                                                        type="button"
                                                        className={`px-3 py-1.5 rounded-full text-[8.5px] font-black uppercase tracking-wider transition-all ${coordFormat === 'DD' ? 'bg-primary text-white shadow-sm' : 'text-on-surface/40 hover:text-on-surface'}`}
                                                        onClick={() => handleFormatChange('DD')}
                                                    >
                                                        DD (Decimal)
                                                    </button>
                                                    <button
                                                        type="button"
                                                        className={`px-3 py-1.5 rounded-full text-[8.5px] font-black uppercase tracking-wider transition-all ${coordFormat === 'DDM' ? 'bg-primary text-white shadow-sm' : 'text-on-surface/40 hover:text-on-surface'}`}
                                                        onClick={() => handleFormatChange('DDM')}
                                                    >
                                                        DDM (Nautical)
                                                    </button>
                                                </div>
                                                
                                                {/* Import from Excel inside modal */}
                                                <input type="file" ref={modalFileInputRef} style={{ display: 'none' }} accept=".xlsx, .xls" onChange={handleModalImportExcel} />
                                                <button 
                                                    type="button" 
                                                    className="flex items-center gap-1.5 bg-white hover:bg-slate-50 border border-surface-low/30 px-3 py-2 rounded-full text-[8.5px] font-black text-on-surface/40 uppercase tracking-widest transition-all" 
                                    ...
                                    <div className="col-span-2 flex flex-col gap-1.5">
                                        <label className="text-[10px] font-black text-on-surface/30 uppercase tracking-widest block px-1">
                                            Vertices Coordinates * (one pair per line)
                                        </label>
                                        <textarea
                                            rows={6}
